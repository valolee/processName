...
- (BOOL)assertion:(int)condition message:(char *)message type:(char *)type {
    if (condition == 0) {
        fprintf(stderr, "ioreg: error: %s.\n", message);
         _lastError= [NSError errorWithDomain:@"ioreg" code:-1
                                  userInfo:@{@"message":(message == NULL)?@"unkown error":[NSString stringWithUTF8String:message],
                                             @"type":(type == NULL)?@"ErrorUnkown":[NSString stringWithUTF8String:type]}];
    }
    return condition == 0;
}
...
- (ELLIOKitNodeInfo *)dumpIOKitTree {
    ...
    assertion(status == KERN_SUCCESS, "can't obtain I/O Kit's master port", "ErrorObtainMasterPort");

    service = IORegistryGetRootEntry(iokitPort);
    assertion(service, "can't obtain I/O Kit's root service", "ErrorObtainRootService");
...
- (ELLIOKitNodeInfo *)_scan:(ELLIOKitNodeInfo *)parent service:(io_registry_entry_t)service options:(struct options)options {
    ...
    assertion(status == KERN_SUCCESS, "can't obtain children", "ErrorObtainChildren");
...
- (ELLIOKitNodeInfo *)_showService:(io_registry_entry_t)service parent:(ELLIOKitNodeInfo *)parent options:(struct options)options {
    ...
    assertion(status == KERN_SUCCESS, "can't obtain name", "ErrorObtainName");
    ...
    if (options.flags & kIORegFlagShowProperties) {
        ...
        assertion(status == KERN_SUCCESS, "can't obtain properties","ErrorObtainProperties");
        assertion(CFGetTypeID(properties) == CFDictionaryGetTypeID(), NULL,"ErrorTypeID");
